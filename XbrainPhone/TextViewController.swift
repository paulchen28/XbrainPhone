...
class TextViewController: UIViewController {
    ...
    @IBOutlet weak var TextView_speech: UITextView!
    
    @IBOutlet weak var text_show: UILabel!
    ...
    @IBAction func voice_button(_ sender: UIButton) {
        
        if sender.tag == 0 {
             startSpeechRecongnition()
             //切換圖示
             sender.configuration?.background.image = UIImage(named: "VoiceCompletion")
             //更改tag等待下一次切換
             sender.tag = 1
             self.text_show.text = "請貼上文字或使用語音輸入記錄"
         }else{
             cancelSpeechRecongnition()
             //切換圖示
             sender.configuration?.background.image = UIImage(named: "Voice")
             //更改tag等待下一次切換
             sender.tag = 0
         }
    }
    
    @IBAction func reEnter(_ sender: UIButton) {
        //重新輸入
        self.TextView_speech.text = ""
        self.text_show.text = "請貼上文字或使用語音輸入記錄"
    }
    
    
    @IBAction func text_save(_ sender: UIButton) {
        //儲存
        //取得現在時間
        ...
        self.text_show.text = "記錄時間 " + timeString
    }
        ...
        task = stranger.recognitionTask(with: self.request){
            ...
            if let e = err{
                ...
                if let real_resulty = result{
                    ...
                    print(ok_result)
                    self.TextView_speech.text = ok_result
                    /*
                ...
                */
                    ...
                    if(last_word == "red"){
                        DispatchQueue.main.async {
                            self.TextView_speech.backgroundColor = UIColor.white
                        }
